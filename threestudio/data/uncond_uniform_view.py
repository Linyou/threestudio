...
@dataclass
class RandomCameraUniformViewDataModuleConfig:
...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    def __init__(self, cfg: Any) -> None:
        super().__init__()
        self.cfg: RandomCameraUniformViewDataModuleConfig = cfg
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        azimuth_deg: Float[Tensor, "Bdivn"] = (
            torch.rand(self.batch_size // self.cfg.n_uniform_views)
            * (self.azimuth_range[1] - self.azimuth_range[0])
            + self.azimuth_range[0]
        )
        uniform_deg: Float[Tensor, "n"] = torch.linspace(
            0, 360, self.cfg.n_uniform_views + 1
        )[: self.cfg.n_uniform_views]
...
@register("random-camera-uniform-view-datamodule")
class RandomCameraUniformViewDataModule(pl.LightningDataModule):
    cfg: RandomCameraUniformViewDataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(RandomCameraUniformViewDataModuleConfig, cfg)
