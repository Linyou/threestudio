...
from threestudio.models.guidance.tensorrt_model import TensorRTModel



@threestudio.register("stable-diffusion-unified-guidance-trt")
class StableDiffusionUnifiedGuidanceTRT(BaseModule):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        # initialize TRT 
        assert self.cfg.controlnet_model_name_or_path is not None and len(self.cfg.controlnet_model_name_or_path) == 1
        self.control_type = self.cfg.controlnet_model_name_or_path[0]
        if self.control_type == "lllyasviel/control_v11p_sd15_canny":
            self.unet_trt = TensorRTModel("/mnt/pfs/users/shenmingzhu/vastcode/submit/FastDiffusion/engines/stable-diffusion-v1-5-canny/unet/model.engine")
        elif self.control_type == "lllyasviel/control_v11f1p_sd15_depth":
            self.unet_trt = TensorRTModel("/mnt/pfs/users/liuxuebo/project/FastDiffusion_mz/stable-diffusion-v1-5-depth/unet/model.engine")
        else:
            raise NotImplementedError
    ...
    def get_eps_pretrain(
        ...
        if prompt_utils.use_perp_neg:
            ...
            assert len(controlnet_inputs) == 1
            with torch.no_grad():
                noise_pred = self.unet_trt(
                    sample=torch.cat([latents_noisy] * 2, dim=0),
                    timestep=t,
                    encoder_hidden_states=text_embeddings,
                    controlnet_conds=controlnet_inputs[0],
                    conditioning_scales=torch.as_tensor([[self.controlnets[0]['scale']]], device=latents_noisy.device, dtype=latents_noisy.dtype)
                )["noise_pred"].clone()
